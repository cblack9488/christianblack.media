...
(async () => {
  ...
  for (const p of ALL) {
    ...
    /* The gallery needs each photograph's shape to lay its rows out before
       anything has downloaded. The browser has just loaded them all, so read
       the natural sizes here rather than parsing JPEG headers. */
    if (p.id === 'photography') {
      const sizes = await page.evaluate(() =>
        [...document.querySelectorAll('.grid img, .grid video')].reduce((m, n) => {
          const src = (n.getAttribute('poster') || n.getAttribute('src') || '')
            .replace(/^.*?(images\/)/, '$1');
          const w = n.naturalWidth || n.videoWidth, h = n.naturalHeight || n.videoHeight;
          if (src && w && h) m[src] = [w, h];
          return m;
        }, {}));
      ...
      console.log('     wrote photo-sizes.js (' + Object.keys(sizes).length + ' frames)');
    }
