...
/* A page's heading and its search-result title do different jobs. "Words"
   is right above the journal; it means nothing in a list of search results.
   seoTitle / seoDescription in content.js override the heading and lede for
   search only, and change nothing on the page itself. */
const PAGES = [
  { file: 'index.html', id: 'journal', title: C.journal.title, kicker: 'Journal',
    seoTitle: SITE.seoTitle, seoDescription: SITE.seoDescription,
...
PAGES.forEach(p => {
  const sec = C[p.id] || {};
  p.seoTitle = p.seoTitle || sec.seoTitle;
  p.seoDescription = p.seoDescription || sec.seoDescription;
});
...
function headBlock(p) {
  ...
  const desc = p.seoDescription || p.desc;
...
/* ---------- picture sizes -------------------------------------------------
   ...
   The gallery needs every photograph's true pixel size before anything has
   downloaded: the justified rows are laid out from the shapes, and each
   full-width frame offers the browser both a grid file and a large one, which
   only works if it is told how wide each of them really is.

   Read from the file headers rather than from a rendered page, so the map
   exists before the first page is prerendered and covers the large files,
   which no page ever puts in an <img>. */
...
function jpegSize(buf) {
  ...
  while (i + 9 < buf.length) {
    ...
    /* SOF0..SOF15, minus the four that are not start-of-frame */
...
function writePhotoSizes(extra) {
  const dir = path.join(ROOT, 'images/photography');
  const sizes = {};
  if (fs.existsSync(dir)) {
    for (const name of fs.readdirSync(dir).sort()) {
      ...
      const wh = ext === '.png' ? pngSize(buf) : jpegSize(buf);
      if (wh && wh[0] && wh[1]) sizes['images/photography/' + name] = wh;
      else console.log('     ! could not read size of ' + name);
    }
  }
  Object.assign(sizes, extra || {});
  fs.writeFileSync(path.join(ROOT, 'assets/js/photo-sizes.js'),
    '/* Generated by tools/build.js — natural pixel sizes, so the gallery can\n' +
    '   lay itself out before the photographs have downloaded, and so each\n' +
    '   full-width frame can tell the browser how wide its two files are. */\n' +
    'window.CB_PHOTO_SIZES = ' + JSON.stringify(sizes, null, 2) + ';\n');
  return sizes;
}

(async () => {
  console.log('  photo sizes: ' + Object.keys(writePhotoSizes()).length + ' files');
  ...
  for (const p of ALL) {
    ...
    /* A moving frame's shape is not in any file header this script reads, so
       take it from the browser, which has just decoded the video, and fold it
       into the map the header pass already wrote. */
    if (p.id === 'photography') {
      const fromPage = await page.evaluate(() =>
        [...document.querySelectorAll('.grid video')].reduce((m, n) => {
          const src = (n.getAttribute('src') || (n.querySelector('source') || {}).src || '')
            .replace(/^.*?(images\/)/, '$1');
          if (src && n.videoWidth && n.videoHeight) m[src] = [n.videoWidth, n.videoHeight];
          return m;
        }, {}));
      const total = Object.keys(writePhotoSizes(fromPage)).length;
      console.log('     photo-sizes.js: ' + total + ' entries (+' +
        Object.keys(fromPage).length + ' from the page)');
    }
